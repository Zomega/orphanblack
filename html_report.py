...
def writeReportToHTML(report, file_name):
        ...
        errors_info = "\n".join(['<P> <FONT COLOR=RED> %s </FONT> </P>' % (error_info.replace('\n', '<BR>'),) for error_info in report._error_info])

        very_strange_const = 'VERY_STRANGE_CONST'

        clone_descriptions = []
        for clone_i in range(len(report._clones)):
            try:
                clone = report._clones[clone_i]
                s = '<P>'
...
clusterize_using_dcup = %s<BR>
        ...
        if report._mark_to_statement_hash:
            ...
            marks = report._mark_to_statement_hash.keys()
            marks.sort(lambda y, x: cmp(len(report._mark_to_statement_hash[x]), len(report._mark_to_statement_hash[y])))
            counter = 0
            for mark in marks[:20]:
                counter += 1
                marks_report += '<BR>' + str(len(report._mark_to_statement_hash[mark])) + ':' + str(mark.getUnifierTree()) + "<a href=\"javascript:unhide('stmt%d');\">show/hide representatives</a> " % (counter,)
                marks_report += '<div id="stmt%d" class="hidden"> <BR>' % (counter,)
                for statement in report._mark_to_statement_hash[mark]:
